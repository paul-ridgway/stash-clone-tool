...
# Doug Added to preserve editing stash tool in place when downloaded by its self
relative_folder = ask("Relative directory e.g. '..': )") { |q| q.echo = true }
...
projects['values'].each do |p|
  ...
  project['values'].each do |r|
    ...
    folder = File.join(relative_folder, p['key'], r['slug'])
